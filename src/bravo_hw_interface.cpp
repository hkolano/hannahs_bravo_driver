...
namespace bravo_base
{
    ...
    void BravoHWInterface::read(const ros::Time& time, const ros::Duration& period)
    {
        ...
        for (uint8_t i = 1; i < 7; ++i){
            // query position, velocity, and current of deviceID i
            joint_positions_[i] = client_.position(i);
            joint_velocities_[i] = client_.velocity(i);
            joint_efforts_[i] = client_.current(i);
        }
        ...
        if (debug_)
        {
            ...
            const char sep = ' ';
            std::stringstream ss;
            ss << std::left << std::setw(width) << std::setfill(sep);
            for (uint8_t i = 1; i < 7; ++i){
                ss << "Joint " << i << std::left << std::setw(width) << std::setfill(sep);
            }
            ss << std::endl;
            for (uint8_t i = 1; i < 7; ++i){
                ss << joint_positions_[i] << std::left << std::setw(width) << std::setfill(sep);
            }
            ss << std::endl;
